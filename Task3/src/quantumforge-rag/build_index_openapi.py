...
USE_CACHE = True  # True = использовать кэш, False = пересоздавать
CACHE_DIR = "cache_chunks"
os.makedirs(CACHE_DIR, exist_ok=True)

# === 1. Эмбеддинг-модель OpenAI ===
EMBEDDING_MODEL = "text-embedding-3-small"
client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
logger.info(f"Используем эмбеддинг-модель OpenAI: {EMBEDDING_MODEL}")

def get_embedding(text: str) -> list:
    """Получаем вектор от OpenAI Embeddings"""
...
if collection_name not in qdrant_collections:
    ...
    qdrant.recreate_collection(
        ...
        vectors_config={"size": 1536, "distance": "Cosine"}  # text-embedding-3-small → 1536 dims
    )
...
for filename in os.listdir(docs_dir):
    ...
    for i, chunk in enumerate(chunks):
        ...
        point_id = str(uuid.uuid4())  # Qdrant принимает int или UUID
...
# === 5. Загрузка в Qdrant ===
BATCH_SIZE = 200  # можно уменьшить при таймаутах
if points:
